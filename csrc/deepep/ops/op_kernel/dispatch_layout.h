...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int r = 0; r < round_; r++) {
            ...
            for (int i = coreIdx_ + 1; i <= aivNum_; ++i) {
                DataCopyPad(onePieceGM_[i * ONE_PIECE], onePieceTensor, tempExpertDataCopyParams);
            }
            ...
            if (coreIdx_ == 47 && rankId_ == 15) {
                AscendC::DumpTensor(onePieceGM_, 237, (aivNum_ + 1) * ONE_PIECE);
            }
            SyncFunc<AscendC::HardEvent::MTE2_S>();
            for (int i = 0; i < tempTokens_; ++i) {
                int32_t validFlag = 0;
                ...
                if (validFlag) {
                    ...
                    if (r == 1) {
                        printf("rankId:%d, coreIdx_:%d, r:%d, preRoundsCount_:%d, preCount:%d, tempCount:%d, validOffset:%d\n",
                        rankId_, coreIdx_, r, preRoundsCount_, preCount, tempBCount, validOffset);
                    }
