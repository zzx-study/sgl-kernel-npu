    ...
    GM_ADDR tpSendCountsOut, GM_ADDR workspaceGM, TPipe *pipe, const MoeDistributeDispatchV2TilingData *tilingData)
{
    ...
    // printf("1.3 [RANK %d AIC %d] Init axisMaxBS_ %d hSizeAlignCombine %d baseWindSize_ %d winDataSizeOffset_ %d
    // windowGM_ %p statusSpaceGm_ %p\n",
    //     epRankId_, aivId_, axisMaxBS_, hSizeAlignCombine, baseWindSize_, winDataSizeOffset_, windowGM_,
    //     statusSpaceGm_);
    PipeBarrier<PIPE_ALL>();
#if defined(ASCENDC_OOM) && ASCENDC_OOM == 1
...
__aicore__ inline void MoeDistributeDispatchV2A5<TemplateMC2TypeFunc>::SetStatus()
{
    ...
    // printf("4.1 [RANK %d AIC %d] Dispatch SetStatus totalExpertNum_ %d startExpertId_ %d endExpertId_ %d
    // sendExpertNum_ %d!\n",
