...
class Buffer:
    ...
    def _hccl_dispatch(
        ...
        num_global_tokens_per_expert = torch.stack(
            num_local_tokens_per_expert_list, dim=0
        )
        ...
        global_input_tokens = torch.empty(
            (recv_buffer_size, hidden_shape[-1]), dtype=x.dtype, device=x.device
        )
        ...
        repeated_expert_ids = (
            local_expert_ids.unsqueeze(0).expand(world_size, -1).contiguous().flatten()
        )
        ...
        global_tokens_indices = torch.repeat_interleave(
            repeated_expert_ids, flat_counts
        )
        ...
        num_recv_tokens_per_expert_list = (
            num_global_tokens_per_local_expert.sum(dim=0).cpu().tolist()
        )
    ...
    def _hccl_combine(
        ...
        hidden_states = torch_npu.npu_moe_token_unpermute(x, reversed_global_mapping)
        ...
        local_tokens = torch.empty(
            (recv_buffer_size, x.shape[-1]), dtype=x.dtype, device=x.device
        )
    ...
    def combine(
        ...
        # --- HCCL Branch ---
        if os.getenv("USE_HCCL_MOE") == "1":
            return self._hccl_combine(x=x, handle=handle, async_finish=async_finish)
        # -------------------
